...
def getCaller(path: str):
    ...
    if len(pathParts) == 3:
        ...
        raise ValueError("{} is not a valid input for this script. Required: 'module/caller/sample_type.vcf'.".format(path))
...
def fixPindel(header: pysam.libcbcf.VariantHeader, row: pysam.libcbcf.VariantRecord):
...
def writeNewVcf(path: str, header: pysam.libcbcf.VariantHeader, vcf: pysam.libcbcf.VariantFile, caller: str):
    ...
    for row in vcf.fetch():
        if caller == "pindel_vcf":
            ...
            raise ValueError("{} is not a valid caller for this script. So far implemented for: pindel_vcf.".format(caller))
        new_vcf.write(row)
    return
